...
- **Pay a one-time reward twice.** Strawberry and friends return to the pool like everything else, so you *can* be offered one again — but its reward fires at most once per run, and the tooltip carries the game's own `This relic was all used up.` line so the offer is never a silent gold trap.
